resource "aws_autoscaling_group" "ASG01" {
  name_prefix = "ASG01-auto-scaling-group-"
  #min_size         = 6
  min_size         = 3
  max_size         = 15
  desired_capacity = 3
  ...
  force_delete              = true
  target_group_arns         = [aws_lb_target_group.ASG01_TG01.arn]
...
resource "aws_autoscaling_attachment" "ASG01_attachment" {
  autoscaling_group_name = aws_autoscaling_group.ASG01.name
  lb_target_group_arn    = aws_lb_target_group.ASG01_TG01.arn
}